...
public class CompletionService extends LanguageService
{
	...
	public CompletionList doComplete(final RexxFile pRexxFile, final Position pPosition,
			final CancelChecker pCancelChecker)
	{
		final int line = pPosition.getLine();
		final int column = pPosition.getCharacter();
		log.debug("complete for {} at ({},{})", pRexxFile.getUri(), line, column);
		final int tokenPosition = getTokenPosition(pRexxFile, line, column);
		log.debug("match with token {}", pRexxFile.getToken(tokenPosition));
		final List<CompletionItem> completionItems = new ArrayList<>();
		addVariablesToCompletions(pRexxFile, tokenPosition, completionItems);
		addLabelsToCompletions(pRexxFile, tokenPosition, completionItems);
		addFunctionsToCompletions(pRexxFile, tokenPosition, completionItems);
		// addKeywordsToCompletions(pRexxFile, tokenPosition, completionItems);
	...
	private int getTokenPosition(final RexxFile pRexxFile, final int pLine, final int pColumn)
	{
		int tokenPosition = pRexxFile.locatePrevToken(pLine, pColumn);
		final RexxToken token = pRexxFile.getToken(tokenPosition);
		if (token.getType() == TokenType.WHITESPACE && token.getLine() == pLine && token.getColumn() == pColumn) {
			if (tokenPosition > 0) {
				--tokenPosition;
			}
		}
	...
	private void addVariablesToCompletions(final RexxFile pRexxFile, final int pTokenPosition,
			final List<CompletionItem> pCompletionItems)
	{
		final RexxToken token = pRexxFile.getToken(pTokenPosition);
	...
	private void addLabelsToCompletions(final RexxFile pRexxFile, final int pTokenPosition,
			final List<CompletionItem> pCompletionItems)
	{
		final RexxToken token = pRexxFile.getToken(pTokenPosition);
		final String tokenText = token.getText();
		for (final String label : pRexxFile.getLabels()) {
	...
	private void addFunctionsToCompletions(final RexxFile pRexxFile, final int pTokenPosition,
			final List<CompletionItem> pCompletionItems)
	{
		final RexxToken token = pRexxFile.getToken(pTokenPosition);
		final String tokenText = token.getText();
	...
	private void addKeywordsToCompletions(final RexxFile pRexxFile, final int pTokenPosition,
			final List<CompletionItem> pCompletionItems)
	{
		final RexxToken token = pRexxFile.getToken(pTokenPosition);
		final String tokenText = token.getText();
